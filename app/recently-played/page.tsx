  ...
  return (
    <ProtectedRoute>
      <main className="min-h-screen bg-black text-white pb-32">
        {/* Premium Hero */}
        <div className="relative overflow-hidden bg-gradient-to-b from-blue-900/40 to-black px-8 pt-24 pb-12 border-b border-white/5">
          <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-blue-600/20 blur-[150px] rounded-full mix-blend-screen" />
          <div className="absolute top-20 left-0 w-[400px] h-[400px] bg-cyan-600/20 blur-[120px] rounded-full mix-blend-screen" />
          ...
          <div className="relative z-10 max-w-7xl mx-auto flex flex-col md:flex-row items-center md:items-end gap-8">
            <motion.div 
              ...
              className="w-48 h-48 md:w-60 md:h-60 shrink-0 rounded-2xl bg-gradient-to-br from-blue-500 via-cyan-500 to-teal-500 flex items-center justify-center shadow-2xl shadow-blue-500/30 border border-white/20"
            >
              <History size={80} className="text-white drop-shadow-xl" />
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.1 }} className="text-center md:text-left">
              <span className="text-xs font-bold tracking-widest uppercase text-cyan-400 mb-2 block">History</span>
              <h1 className="text-5xl md:text-8xl font-black tracking-tighter mb-6">Recently Played</h1>
              <div className="flex flex-wrap items-center justify-center md:justify-start gap-2 text-sm text-zinc-300 font-medium">
                ...
                <span>•</span>
                <span>{uniqueRecentSongs.length} songs</span>
              </div>
            </motion.div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-8 py-8">
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.2 }} className="flex items-center gap-4 mb-10">
              <motion.button
                ...
                onClick={() => {
                  ...
                  const firstSong = uniqueRecentSongs[0];
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                onClick={() => {
                  const shuffled = [...uniqueRecentSongs].sort(() => Math.random() - 0.5);
                  setQueue(shuffled);
          ...
          {uniqueRecentSongs.length === 0 ? (
            ...
            >
              ...
              <h2 className="text-3xl font-bold mb-3 text-white">No history yet</h2>
              <p className="text-zinc-400 max-w-md mb-8">Start listening to some music and your recent activity will show up here.</p>
              <Link href="/">
            ...
            <motion.div variants={container} initial="hidden" animate="show" className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
              {uniqueRecentSongs.map((song: any, index: number) => (
